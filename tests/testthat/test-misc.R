...
test_that("no pairable data", {
  ...
  params = list(PairedData = TRUE,
                DataCrop = list(Start=1, End=4),
                GroupingKeyWords = c("test1", "test2"),
                Sheet = "test")
  expect_message(manipulate_data(df, params=params, ana_name="test"),
...
test_that("manipulate_data() normalizes absolute", {
  ...
  expect_equal(manipulate_data(df, params=params, ana_name="test"), result)
})

  ...
  methods = list(initialize = function()
  {
    callSuper(
      ...
      plot_fnc = function(.self, data)
      {
        ...
        data_columns = get_bool_for_columns_by_key(names(data), params$Key, "Time")
        ...
        for (key in names(data_columns))
        {
          ...
          {
            if (params$CropTrace > 0)
            {
              key_df = key_df %>%
                filter(.data$Time > .data$Time[1]+params$CropTrace)
            }
            ...
            t_plot$file_name = paste0(.self$ana_name, "_Trace_", key, ".png")
            t_plot$width = 2
          ...
          if (params$PeakAverage)
          {
            ...
            df_average = data.frame(Time = seq(0-params$before,0+params$after, 1.0/sum(key_df$Time < key_df$Time[1]+1)))
            ...
            for (stim in params$Stimuli)
            {
              ...
              for(elem in names(time_of_max))
              {
                ...
                df_stims = cbind(df_stims, extract_values_between_two_given_times(tmp,
                ...
                if (params$ControlPlots)
                {
                  ...
                  c_plot$file_name = paste0(.self$ana_name, "control_", time_of_max[[elem]], "_", elem, ".png")
                  c_plot$width = 0.5
              ...
              c_plot$file_name = paste0(.self$ana_name, "_control_avg", stim, "_", key, ".png")
              c_plot$width = 0.5
              plotl[[c_plot$file_name]] = c_plot
            ...
            a_plot$file_name = paste0(.self$ana_name, "_PeakAvg_", key, ".png")
            a_plot$width = 0.5
